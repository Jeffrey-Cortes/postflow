...
export class OpenAiDraftGeneratorService implements DraftGenerator {
  ...
  async generate(input: DraftGenerationInput): Promise<GeneratedDraft> {
    ...
    const response = await client.responses.create({
      ...
      instructions:
        'Write an institutional Spanish social media draft. Never invent names, dates, places, titles, URLs, or facts. Use only source material. Historical posts are style reference only; never copy them verbatim. Return JSON matching the schema.',
      input: `Platform: ${input.platform}\nSource material:\n${input.sourceText}\n\nStyle references:\n${input.references.map((reference) => `- ${reference.text}`).join('\n') || '(none)'}\n\nAvailable image IDs: ${input.availableImageFileIds.join(', ') || '(none)'}`,
      text: {
        format: {
          type: 'json_schema',
          name: 'social_draft',
          ...
          schema: {
            ...
            properties: {
              ...
              selectedImageFileIds: {
                type: 'array',
                items: { type: 'string', enum: input.availableImageFileIds },
                maxItems: Math.min(4, input.availableImageFileIds.length),
              },
